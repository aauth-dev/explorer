...
const CLAIM_TOOLTIPS: Record<string, string> = {
  typ: "Token type identifier. aa-agent+jwt = Agent Token, aa-person+jwt = Person Token, aa-resource+jwt = Resource Token, aa-auth+jwt = Auth Token",
  iss: "Issuer — who created and signed this token",
  sub: "Subject. In an agent token, the agent identifier. In a person, resource or auth token, the person's directed identifier — opaque, unique within the issuer. Identity is the pair (iss, sub); a sub from one issuer MUST NOT be matched against a record established under another.",
  aud: "Audience — which party should accept this token",
  iat: "Issued At — Unix timestamp when the token was created",
  exp: "Expiration — Unix timestamp after which the token is invalid",
  jti: "JWT ID — unique identifier for this token (replay detection, audit, revocation)",
  dwk: "Discovery Well-Known — the metadata document name for key discovery (e.g. aauth-agent.json, aauth-person.json)",
  ...
  approver: "HTTPS URL of the entity that approved the mission. A mission blob member; it appears in no token.",
  s256: "The mission identifier — the digest of the exact bytes the PS persisted as the mission blob",
  tenant: "Tenant identifier declaring the organization the person belongs to. Organizational context, not part of the identifier, which is (iss, sub).",
  account: "The account at the resource this authorization is for, in the resource's own namespace",
  parent_agent: "Sub-agent marker — the identifier of this agent's parent. Its presence makes this a sub-agent's token.",
  resource: "Resource identifier this token grants access to",
  r3_uri: "URI where the AS or PS can fetch the R3 document. Agents cannot fetch it.",
  r3_s256: "SHA-256 of the R3 document's bytes as served (base64url, unpadded). The hash is the document's identity, not the URI — it enables infinite caching and permanent audit records.",
  r3_granted: "Operations fully authorized. The resource serves these directly from the token, with no introspection and no R3 fetch.",
  r3_per_call: "Operations authorized in principle but not for any specific call. The resource builds a per-call proposal from the actual parameters and challenges before serving. Named r3_conditional before R3 -02.",
};
